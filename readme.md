...
- No hay historial de corridas: cada ejecución sobreescribe los JSON en [results/](results) (`B3_static.json`, `B7_dynamic_attacks.json`, etc.), así que no existe forma de comparar la corrida de hoy con la de ayer ni de ver cómo evolucionaron los hallazgos entre corridas. Ver punto 13 de la sección 7 para el plan de agregar esto.
...
13. Agregar historial de corridas (base de datos) y una vista de "corridas pasadas" en el frontend. — Pendiente, decidido el 2026-08-02. Hoy cada corrida sobreescribe los JSON en [results/](results) (ver punto nuevo en la sección 5), así que no hay forma de comparar resultados en el tiempo — la forma en que herramientas reales de este tipo (Burp Suite, OWASP ZAP, Nessus, etc.) resuelven esto es con una tabla de corridas (`runs`: id, fecha, modo, estado) y los hallazgos vinculados por `run_id` en vez de un único snapshot. Alcance decidido: SQLite (alcanza sobradamente a esta escala, sin agregar infraestructura nueva) más una pestaña "Past Runs" en el frontend que liste corridas anteriores y reutilice `PipelineView`/`CorrelationView` apuntando a datos históricos en vez de solo a la corrida en curso. Los gráficos de tendencia entre corridas (hallazgos nuevos vs. recurrentes, distribución de severidad en el tiempo) quedan fuera de este alcance, como posible extensión futura si el tiempo lo permite.
