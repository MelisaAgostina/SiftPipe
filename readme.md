...
- Estado: ⚡ Parcial — resiliencia corregida el 2026-07-31 (ver [fixes.txt](fixes.txt), sección "SESSION 3"); headless real y selectores de login con fallback agregados el 2026-08-04; interstitial de landing page automatizado el 2026-08-06 (ver [fixes.txt](fixes.txt), sección "SESSION 7").
- Observaciones:
  - El flujo sigue dependiendo de la UI de Mattermost y de selectores concretos para rutas y formularios, por lo que sigue siendo frágil ante cambios en la interfaz — eso no cambió esta sesión. Lo que sí se corrigió el 2026-08-04: los dos selectores de login con más impacto (email/password) ahora tienen fallback en vez de ser un único punto de falla — ver [blocks/mattermost_auth.py](blocks/mattermost_auth.py). Una reescritura genérica del discovery en sí (independiente de Mattermost) sigue siendo un cambio de fondo, no un ajuste (ver punto 1 de la sección 8).
...
- Estado: 🔧 Implementado y probado end-to-end contra una instancia Mattermost real (ver [fixes.txt](fixes.txt)); captura de HTTP corregida el 2026-07-31 (ver [fixes.txt](fixes.txt), sección "SESSION 3"); video por payload y headless real agregados el 2026-08-04; las reglas de detección tienen cobertura de tests con un browser simulado.
- Observaciones:
...
- Observaciones:
  - No verificado con navegador real en ninguna sesión hasta ahora (no había herramienta de automatización de browser disponible): se confirmó que el HTML server-renderizado no tiene errores, que las respuestas del backend calzan exactamente con los tipos TypeScript, que `tsc --noEmit` y `eslint` pasan limpio en cada archivo tocado, y que los endpoints nuevos responden correctamente vía `TestClient`/smoke tests directos — pero sigue faltando la confirmación visual de que los datos pintan bien después de la hidratación, y el click-through completo de una corrida real (correr pipeline → aprobar payloads en la UI → ver que B7-B9 continúa → ver el video/screenshot de un finding → revisarlo después en Past Runs).
  - `TopBar.tsx` sigue hardcodeado (sin endpoint de backend para info de entorno real) — no se tocó esta ronda.
...
- [tests/](tests/) ya no está vacío: hay 113 tests reales (`python -m unittest discover -s tests`) cubriendo B3 (incluyendo la corrección de códigos OWASP), B4 (post-procesamiento puro + `_determine_status`), B5, B6, B7 (con un browser Playwright simulado, incluyendo el predicado de captura HTTP, el caso de timeout, y el contexto por-payload), B8, B9 (matching por CWE/OWASP, juez LLM, scoring), [blocks/taxonomy.py](blocks/taxonomy.py), [blocks/scoring.py](blocks/scoring.py), [blocks/mattermost_auth.py](blocks/mattermost_auth.py) y [blocks/run_history.py](blocks/run_history.py), además de las rutas de [api.py](api.py). B0–B2, B10–B12 no tienen código propio, así que no aplican.
...
5. Correr los tests
   ...
   - Son 113 tests en total al 2026-08-06 (ver [tests/](tests/)).
