...
export const es: Strings = {
  ...
  findingRow: {
    ...
    severityStatLabel: "Severidad",
    typeStatLabel: "Tipo",
    scoreStatLabel: "Puntaje",
    confidenceStatLabel: "Confianza",
    matchRationaleToggle: "Motivo de la coincidencia",
  },
  pastRunsView: {
    downloadReport: "Descargar informe",
    viewRawJson: "Ver JSON sin procesar",
    reportButtonLabel: "Informe",
  ...
  scopeReviewView: {
    pausedForReview:
      "Este es un target recién descubierto — todavía nadie revisó a qué puede acceder este login. Marca las páginas que son seguras para seguir analizando; lo que quede sin marcar se descarta antes de que corra cualquier paso de ataque.",
    noPagesYet: "Todavía no se descubrió ninguna página.",
    selectedCount: (selected, total) => `${selected} de ${total} páginas seleccionadas`,
    selectAll: "Seleccionar todas",
    deselectAll: "Deseleccionar todas",
    noFormsOrInputs: "No se encontraron formularios ni campos en esta página",
    endpointsHeading: "Otros endpoints detectados (solo informativo)",
    approveAndContinue: (count) => `Aprobar ${count} página(s) y continuar`,
    approvalSent: "Alcance aprobado — continuando con la generación de payloads",
    couldNotApprove: (detail) => `No se pudo aprobar: ${detail}`,
    unknownError: "error desconocido",
  },
  discoverTargetDialog: {
    triggerButton: "Agregar nuevo target",
    title: "Descubrir un nuevo target",
    stepOf: (step, total) => `Paso ${step} de ${total}`,
    stepIdentity: "¿Cuál es el nombre del target?",
    stepLocation: "¿Dónde vive el target?",
    stepCredentials: "Cómo acceder al target",
    nameLabel: "Nombre del target",
    namePlaceholder: "ej. juiceshop",
    invalidName: "Solo minúsculas, dígitos, guiones bajos y guiones.",
    nameChecking: "Verificando…",
    nameAvailable: "Disponible",
    nameTaken: "Ya existe un target con este nombre.",
    baseUrlLabel: "URL base",
    baseUrlHelper: "La URL base más el login path es la página que SiftPipe abre primero.",
    loginPathLabel: "Login path",
    invalidUrl: "Ingresa una URL completa sin path acá, ej. http://localhost:3000",
    invalidLoginPath: "El login path debe empezar con /, ej. /login",
    credentialsHelper:
      "Nombra las dos variables de entorno que tienen las credenciales. Nunca vas a escribir una contraseña acá — SiftPipe lee el valor directamente de tu propio archivo .env.",
    usernameEnvLabel: "Variable de usuario",
    passwordEnvLabel: "Variable de contraseña",
    invalidEnvVarName: "Los nombres de variable solo pueden usar A-Z, 0-9 y guiones bajos.",
    envVarChecking: "Verificando…",
    envVarPresent: "Encontrada en tu entorno",
    envVarMissing: "Todavía no está en tu .env — agrégala ahí antes de correr el pipeline.",
    back: "Atrás",
    next: "Siguiente",
    cancel: "Cancelar",
    startDiscovery: "Iniciar descubrimiento",
    runningTitle: "Llegando a la página de login…",
    runningBody:
      "Las credenciales se leen directamente de tu entorno — SiftPipe nunca ve el valor.",
    couldNotStart: (detail) => `No se pudo iniciar el descubrimiento: ${detail}`,
    successTitle: (name) => `${name} descubierto`,
    loginSucceeded: "El login funcionó. Ya se puede seleccionar desde el selector de targets.",
    loginFailed: (error) =>
      `El login no funcionó: ${error}. El archivo del target se guardó igual — podés corregir la página de login o las credenciales e intentar de nuevo con otro nombre.`,
    selectorsFoundHeading: "Selectores encontrados",
    addAnother: "Agregar otro",
    switchToIt: "Cambiar a este",
    close: "Cerrar",
    unknownError: "error desconocido",
  },
  queryState: {
